...
**Psy Models** — secondary track. The clinical-protection segment under `src/demo/clinical/` puts Vanguard in front of **QVAC MedPsy** (`qvac/MedPsy-{4B,1.7B}-GGUF`, Apache 2.0), Tether's own medical-specialized Psy model, running locally as the protected host. The default is **MedPsy-1.7B** (`node src/demo/clinical/run.mjs`) — a 1.28 GB model with clean, direct patient answers, chosen for reproducibility on the judges' replay; `--host medpsy-4b` swaps in the more advanced 4B reasoning model, `--host medgemma` swaps in MedGemma 4B. MedPsy is a strong clinical responder but, like any instruction-tuned model, engages with prompt-injection rather than hard-refusing it — exactly the 94.4% medical-LLM compromise rate reported in JAMA Network Open [verify]. Vanguard catches the injection before MedPsy ever runs the prompt; the audit log is the proof. Verified end-to-end: 10/10 on the canonical battery (6/6 attacks blocked, 4/4 benign answered) with both MedPsy-1.7B and MedPsy-4B as host.
