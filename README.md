...
**Model usage & coverage** — Vanguard is model-plural by design. It orchestrates four distinct QVAC SDK models across six on-device roles: Qwen3 1.7B (LoRA-tuned as the classifier, and base-weights as the self-hardening loop's red-team generator), MedGemma 4B (the protected clinical host, and the loop's referee), Qwen3-VL 2B (vision-describe for image input), and an OCR model (document text). The clinical-protection segment under `src/demo/clinical/` shows Vanguard guarding MedGemma 4B against canonical medical prompt-injection attacks — the 94.4% success rate reported against medical LLMs in JAMA Network Open [verify] — with the audit log proving Vanguard catches the injection before MedGemma sees the prompt. Note: the QVAC SDK ships no dedicated "Psy" model, so Vanguard does not claim the Psy-models track; it competes on General Purpose and demonstrates breadth of model usage there.
...
Two scenario sets ship in this repo: `src/demo/clinical/scenarios.jsonl` (the canonical demo, prompts kept short enough to fit MedGemma's working context window — `node src/demo/clinical/run.mjs` writes results to `artifacts/clinical_demo/session.json`) and `src/demo/clinical/scenarios_hard.jsonl` (longer medical-pretext variants that probe the EXFILTRATION F1 weakness disclosed in `artifacts/training/eval_report.json`). The hard set is the honest stress test; the canonical set is what runs cleanly end-to-end on the hardware in `artifacts/hardware.json`.
