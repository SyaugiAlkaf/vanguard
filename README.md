...
**Psy Models** — secondary track. The clinical-protection segment under `src/demo/clinical/` puts Vanguard in front of **QVAC MedPsy-1.7B** (`qvac/MedPsy-1.7B-GGUF`, Apache 2.0), Tether's own medical-specialized Psy model, running locally as the protected host (`node src/demo/clinical/run.mjs` — default `--host medpsy`; `--host medgemma` swaps in MedGemma 4B). MedPsy is a strong clinical responder but, like any instruction-tuned model, engages with prompt-injection rather than hard-refusing it — exactly the 94.4% medical-LLM compromise rate reported in JAMA Network Open [verify]. Vanguard catches the injection before MedPsy ever runs the prompt; the audit log is the proof.

**Model usage & coverage** — Vanguard is model-plural by design. It orchestrates five distinct QVAC models across on-device roles: Qwen3 1.7B (LoRA-tuned as the classifier, and base-weights as the self-hardening loop's red-team generator), MedPsy-1.7B (the protected clinical Psy host), MedGemma 4B (alternate clinical host and the loop's referee), Qwen3-VL 2B (vision-describe for image input), and an OCR model (document text).

The demo scenarios are written for one user. Sari is 31, post-infection 18 months, dysautonomia and post-exertional malaise, raising a 14-month-old in Surabaya. The only BPJS-covered Long COVID clinic is in Jakarta — she cannot drive there. She runs MedPsy-1.7B on her own laptop because cloud telehealth in Indonesia routinely ships her PII to providers outside the country's PDP jurisdiction, and because Long COVID care literature is mostly in English and mostly behind paywalls she cannot afford. A prompt injection from a URL she clicked, an extension she trusted, or a screenshot a forum mod posted can rewrite the model's behavior mid-consultation: confirming graded exercise therapy is correct (it is contraindicated for ME/CFS-spectrum Long COVID), advising self-medication with benzodiazepines for POTS, or leaking the audit log of her previous queries to whatever code path the attacker controls. Vanguard blocks those before the model ever runs the prompt. The session log in `artifacts/clinical_demo/session.json` is the proof. Sovereignty is not a slogan for Sari. It is the only model that exists for her.
