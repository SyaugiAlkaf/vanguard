...
**Psy Models** — secondary track. The clinical-protection segment under `src/demo/clinical/` puts Vanguard in front of **QVAC MedPsy** (`qvac/MedPsy-{4B,1.7B}-GGUF`, Apache 2.0), Tether's own medical-specialized Psy model, running locally as the protected host. The default is the more capable **MedPsy-4B** (`node src/demo/clinical/run.mjs`; `--host medpsy-1.7b` for the lighter 1.28 GB model with more consistently terse answers, `--host medgemma` for MedGemma 4B). MedPsy is a strong clinical responder but, like any instruction-tuned model, engages with prompt-injection rather than hard-refusing it — exactly the 94.4% medical-LLM compromise rate reported in JAMA Network Open [verify]. Vanguard catches the injection before MedPsy ever runs the prompt; the audit log is the proof. Verified end-to-end: 10/10 on the canonical battery (6/6 attacks blocked, 4/4 benign answered) with MedPsy-4B as host.

**Model usage & coverage** — Vanguard is model-plural by design. It orchestrates six distinct QVAC models across on-device roles: Qwen3 1.7B (LoRA-tuned as the classifier, and base-weights as the self-hardening loop's red-team generator), MedPsy-4B and MedPsy-1.7B (protected clinical Psy hosts, 4B default), MedGemma 4B (alternate host and the loop's referee), Qwen3-VL 2B (vision-describe for image input), and an OCR model (document text).
